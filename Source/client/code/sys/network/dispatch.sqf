/*
	...
	rpclSystemNetworkDispatch

	This function was swiped from the ExileMod Project
	--------------------------------------------------------------------------------
...
*/
private [
	'_payload', 'messageName', '_messageParameters', '_config', '_allowedParameters',
	'_moduleName', '_functionName', '_functionCode', '_message'
];

_payload = _this;

try {
	if(isNil '_payload') then { throw 'Message payload is not defined!'; };
	if(typeName _payload != "ARRAY") then { throw 'Message payload is not an array!'; };
	if(count _payload != 2) then { throw format ['Wrong envelope field count!  Payload: %1', _payload]; };
	...
	if(isClass (missionConfigFile >> 'CfgNetworkMessages' >> _messageName)) then {
		...
		} else { throw format ['Forbidden message name! Payload: %1', _payload]; };
	};
	...
	_functionName = format ['rpcl%1Network%2', _moduleName, _messageName];
	...
	format ['NetworkManager - %1', _exception] call rpclLog;
};
